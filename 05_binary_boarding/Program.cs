...
namespace _05_binary_boarding
{
    ...
    class Program
    {
        ...
        private static void ShowSeating(int[,] seating)
        {
            string output;
            int result = -1;
            ...
            for (int j = 0; j < 128; j++)
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    if (j > 1 && j < 127 && i > 1 && i < 7)
                    {
                        if (seating[i, j] == 0)
                        {
                            if (seating[i -1, j] > 0 && seating[i + 1, j] > 0 &&
                                seating[i, j - 1] > 0 && seating[i, j + 1] > 0)
                            {
                                result = j * 8 + i;
                            }
                        }
                    }
                    output += seating[i, j].ToString("D3") + " ";
            ...
            Console.WriteLine($"My seat id : {result}");
        }
